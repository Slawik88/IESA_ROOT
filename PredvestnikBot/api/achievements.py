"""
api/achievements.py — Система достижений.

Архитектура:
  1. ACHIEVEMENTS — единственный источник истины. Список всех достижений,
     каждое имеет уникальный key, type (тип события), threshold (порог).

  2. check_and_award(user_id, chat_id, ach_type, value) — ЕДИНСТВЕННЫЙ
     способ выдать достижение. Вызывается в момент события. Никогда не
     вызывается при чтении/просмотре.

  3. get_all_achievements_with_status(user_id, chat_id) — ТОЛЬКО ЧТЕНИЕ.
     Возвращает прогресс для мини-апп. Не выдаёт награды, не изменяет БД.

  4. Выдача атомарна: INSERT INTO user_badges ON CONFLICT DO NOTHING,
     затем выплата (мора + XP). Если INSERT вернул 0 строк — достижение
     уже есть, ничего не делать. Нет catch-up, нет бесконечных тиров.
"""
...
# Обязательные поля: key, title, emoji, description, mora, xp, type, threshold
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Быстрые справочники (строятся один раз при импорте)
# ─────────────────────────────────────────────────────────────────────────────
...
# Типы с булевым значением (0 или 1), без прогресс-бара
BOOL_TYPES: frozenset[str] = frozenset({"married", "has_pet"})

# Метаданные категорий для UI
...
async def _try_insert_badge(user_id: int, chat_id: int, badge_key: str) -> bool:
    ...
    Возвращает True если бейдж только что создан (INSERT выполнен),
    ...
    async with postgres_connect() as db:
        ...
        # asyncpg: result is a string like "INSERT 0 1" or "INSERT 0 0"
...
async def _pay_reward(user_id: int, chat_id: int, mora: int, xp: int) -> None:
    ...
    if mora > 0:
        try:
            ...
            # update_earned=False: награды за достижения не влияют на total_earned
            await _add_mora(user_id, 0, mora, update_earned=False)
        except Exception as e:
            _log.warning("Achievement mora payout failed uid=%s: %s", user_id, e)
    ...
    if xp > 0:
        try:
            ...
            _log.warning("Achievement xp payout failed uid=%s: %s", user_id, e)
...
async def check_and_award(
    ...
    """Единственный публичный способ выдать достижения.

    Проверяет все достижения типа ach_type где threshold <= value.
    Выдаёт только новые (INSERT ON CONFLICT DO NOTHING — идемпотентно).
    Возвращает список только что выданных достижений.

    Безопасно вызывать fire-and-forget: никогда не пробрасывает исключений.
    """
    ...
    for ach in candidates:
        if value < ach["threshold"]:
            break  # список отсортирован → дальше только выше порог
        try:
            is_new = await _try_insert_badge(user_id, chat_id, ach["key"])
            if not is_new:
                continue
            await _pay_reward(user_id, chat_id, ach.get("mora", 0), ach.get("xp", 0))
            newly_awarded.append(ach)
        except Exception as e:
            _log.warning("check_and_award error uid=%s key=%s: %s", user_id, ach.get("key"), e)
...
async def _notify_achievements(
    ...
    """Отправить уведомление о новых достижениях в чат."""
    if not bot:
        try:
    ...
    try:
        ...
        await bot.send_message(
            chat_id,
            f"🏆 <b>{name}</b> разблокировал {header}!\n\n{lines}",
            parse_mode="HTML",
        )
    except Exception as e:
        _log.debug("Achievement notify failed: %s", e)


# ─────────────────────────────────────────────────────────────────────────────
# Публичный API — чтение прогресса (только для мини-апп, БД не изменяется)
# ─────────────────────────────────────────────────────────────────────────────

async def _fetch_earned(db, user_id: int, chat_id: int) -> dict[str, str]:
    """Получить множество выданных бейджей {badge_key: obtained_at} из БД."""
    rows = await db.fetch(
...
async def _fetch_counters(db, user_id: int, chat_id: int) -> dict[str, int]:
    ...
    При ошибке одной из таблиц — логируем и продолжаем с нулём.
    """
    c: dict[str, int] = {}

    # user_mora: gacha, coinflip, rep, expeditions, chests
    try:
    ...
    # boss_damage — суммарный урон за все сессии
    try:
        row = await db.fetchrow(
    ...
    # married (bool — глобально, не per-chat)
    try:
        row = await db.fetchrow(
...
async def get_all_achievements_with_status(user_id: int, chat_id: int) -> dict:
    """Вернуть прогресс достижений для мини-апп.
    ...
    try:
        ...
        _log.warning("get_all_achievements_with_status DB error uid=%s: %s", user_id, e)
    ...
    for ach_type, tiers in ACH_BY_TYPE.items():
        ...
        ranks: list[dict] = []
        unlocked_count = 0
        next_rank_idx: int | None = None  # индекс первого не-разблокированного в ranks
        ...
        # Прогресс до следующего ранга в процентах
        if is_bool:
...
async def get_global_achievements_leaderboard(chat_id: int, limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        _log.warning("get_global_achievements_leaderboard chat=%s: %s", chat_id, e)
        return []
