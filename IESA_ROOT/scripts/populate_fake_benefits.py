#!/usr/bin/env python
"""
Скрипт для создания фейковых бенефитов для членов ассоциации.
Удаляет текущие и создает новые привлекательные преимущества.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("СОЗДАНИЕ ФЕЙКОВЫХ БЕНЕФИТОВ ДЛЯ ЧЛЕНОВ АССОЦИАЦИИ")
        self.stdout.write("=" * 80)
        ...
        self.create_fake_benefits()
        
        self.stdout.write("\n" + "=" * 80)
        self.stdout.write(self.style.SUCCESS("✨ ГОТОВО! Все бенефиты успешно обновлены на сайте"))
        self.stdout.write("=" * 80)

    def clear_benefits(self):
    """Удалить все текущие бенефиты."""
    deleted_count, _ = MemberBenefit.objects.all().delete()
        self.stdout.write(f"✅ Удалено {deleted_count} бенефитов")

    def create_fake_benefits(self):
    """Создать фейковые привлекательные бенефиты для членов."""
    ...
    for benefit_data in benefits_data:
        ...
        print(f"✅ Создан: {benefit.title}")
    
    print(f"\n🎉 Всего создано {len(created_benefits)} бенефитов!")
    return created_benefits


def main():
    print("=" * 80)
    print("СОЗДАНИЕ ФЕЙКОВЫХ БЕНЕФИТОВ ДЛЯ ЧЛЕНОВ АССОЦИАЦИИ")
    print("=" * 80)
    ...
    # Шаг 2: Создать новые
    print("\n📋 Шаг 2: Создание новых фейковых бенефитов...")
    created = create_fake_benefits()
    
    print("\n" + "=" * 80)
    print("✨ ГОТОВО! Все бенефиты успешно обновлены на сайте")
    print("=" * 80)
