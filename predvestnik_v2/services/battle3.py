...
def _best_enemy_plan(state, ei):
    ...
    for cell, movecost in reach.items():
        ...
        for ti, ally in enumerate(allies):
            ...
            strat = 0.0
            # Таунт танка (intercept_all): если враг ДОСТАЁТ до таунтера (цикл и так
            # перебирает только достижимые+в LoS цели) — он обязан бить именно его.
            # Раньше intercept_all читался только в _pick_target (ульты союзника), а
            # реальный вражеский ИИ его игнорировал — таунк был мёртвым кодом (block 6).
            # Большой бонус доминирует над «добить раненого»/«давить саппорта», т.е.
            # таунтер выбирается всегда, когда достижим; вне досягаемости — обычная цель.
            if ally.get("statuses", {}).get("intercept_all"):
                strat += 500.0
