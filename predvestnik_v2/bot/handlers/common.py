...
HELP_PAGES = {
    ...
    "main": (
        ...
        "📱 <b>Mini App</b> — профиль, семья, история операций и Центр активностей.\n"
        "Открой его командой <code>бот сайт</code>.\n"
        "<i>В чате остаются команды общения, групповые игры и полноценное управление чатами.</i>\n\n"
        ...
        "🎮 <b>Активности</b> — Ритм, Сапёр и Мафия\n"
        "⚙️ <b>Управление чатами</b> — модерация, настройки, права и чистка\n"
        "👥 <b>Социальное</b> — профиль, брак, варпы, топы\n"
        "🌑 <b>Тёмная Мора</b> — сохранённый баланс и архивный каталог\n"
        "🐾 <b>Питомцы и события</b> — проектируются отдельно и пока не запущены\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "💡 <b>Коротко о кошельке:</b> старые игровые способы получения ценностей закрыты;\n"
        "новые правила наград и покупок появятся только после отдельного утверждения.\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    "economy": (
        ...
        "  Старые пассивные начисления закрыты. Новые источники и траты будут опубликованы\n"
        "  вместе с утверждённой экономикой.\n\n"
        "💎 <b>Алмазы</b> и 🌑 <b>Тёмная Мора</b>\n"
        "  Исторические остатки сохранены. Новые правила их получения и применения ещё не утверждены.\n\n"
        "📦 <b>Сохранённые вещи</b>\n"
        "▸ <code>бот инвентарь</code> — посмотреть архивные предметы и права.\n"
        "  Их нельзя случайно потратить в старых механиках.\n\n"
        "🎫 <b>Прочее</b>\n"
    ...
    "gacha": (
        ...
        "🎮 <b>Игры</b>\n"
        "▸ <code>бот игры</code> — открыть хаб игр\n"
        "▸ <code>бот мафия</code> — создать чатовую партию в группе\n"
        "  Доступны Ритм, Сапёр и Мафия; старые игры со ставками закрыты.\n\n"
    ...
    "pets": (
        "🐾 <b>ПИТОМЦЫ</b>\n\n"
        "Новая система питомцев готовится отдельно: карточки, уход, отряд, походы и экспедиции появятся только после утверждения правил.\n\n"
        "Прежние питомцы, имена и редкость сохранены в истории. Старые походы и развитие не запускаются."
    ),
    ...
    "social": (
        ...
        "  💡 Семейный кошелёк поддерживает Мору, Алмазы, Тёмную Мору и Зарники;\n"
        "  каждое движение записывается в историю операций.\n\n"
        "🎮 <b>Игры</b>\n"
        "▸ <code>бот игры</code> — открыть хаб игр\n"
        "  Доступны Ритм, Сапёр и чатовая Мафия. Старая клеточная боёвка закрыта.\n\n"
    ...
    "items": (
        ...
        "▸ Новая система питомцев будет показана после утверждения правил"
    ),
    ...
    "events": (
        ...
        "Новый календарь появится только после утверждения игровых событий и экономики. Каждая награда будет иметь заранее опубликованное условие.\n\n"
        "▸ <code>бот игры</code> — открыть доступные игры\n"
...
# Release chat help is intentionally limited to registered, current surfaces.
# Keeping this override next to the keyboard makes stale archive pages impossible
# to discover without deleting historical data needed by compensation/support.
HELP_PAGES.update({
    "main": (
        "🔮 <b>ПРЕДВЕСТНИК</b>\n"
        "<i>Помощник для общения, управления чатами и совместных игр.</i>\n\n"
        "Выбери раздел кнопкой — команды не нужно запоминать.\n\n"
        "👤 <code>бот я</code> — личная карточка\n"
        "🏆 <code>бот топ</code> — сообщения в чате, глобально и топ чатов\n"
        "🎮 <code>бот игры</code> — Мафия, Ритм и Сапёр\n"
        "💍 <code>бот семья</code> — пара и семейные функции\n"
        "🛡 <code>бот помощь</code> — этот справочник"
    ),
    "activity": (
        "🏆 <b>АКТИВНОСТЬ</b>\n\n"
        "Топы считают только сообщения: без валюты, силы и наград за спам.\n\n"
        "Доступны три масштаба:\n"
        "• игроки этого чата;\n• игроки по всем чатам;\n• чаты целиком.\n\n"
        "Периоды: сегодня, неделя, месяц и всё время.\n"
        "Открой: <code>бот топ</code>."
    ),
    "games": (
        "🎮 <b>ИГРЫ</b>\n\n"
        "🔫 <code>бот мафия</code> — групповая игра прямо в чате\n"
        "🎵 Ритм и 💣 Сапёр — через <code>бот игры</code>\n\n"
        "Старые казино, ставки, аукцион, торговец и случайные чат-ивенты закрыты."
    ),
    "social": (
        "👥 <b>СОЦИАЛЬНОЕ</b>\n\n"
        "👤 <code>бот я</code> — профиль\n"
        "💍 <code>бот брак</code> — предложение ответом или по @username\n"
        "🏦 <code>бот семья</code> — семейная карточка\n"
        "🎁 <code>бот подарки</code> — памятные подарки без игровой силы\n"
        "🤝 <code>бот варпы</code> — доступные социальные действия\n\n"
        "Любое необратимое действие требует отдельного подтверждения."
    ),
    "settings": (
        "⚙️ <b>УПРАВЛЕНИЕ ЧАТОМ</b>\n\n"
        "<code>бот настройки чата</code> — права, функции и часовой пояс\n"
        "<code>бот инфо чата</code> — состав и привязка админ-чата\n"
        "<code>бот обновить права</code> — синхронизация Telegram-админов\n"
        "<code>бот чистка</code> — проверка активности без автонаказаний\n\n"
        "Настройки доступны только действующему Telegram-администратору с нужным локальным рангом."
    ),
})
...
def get_help_keyboard(active_tab: str = "main", is_dev: bool = False, user_id: int = 0) -> types.InlineKeyboardMarkup:
    ...
    b.button(text=label("🏆 Активность", "activity"), callback_data=HelpCallback(tab="activity", user_id=user_id))
    b.button(text=label("🎮 Игры", "games"), callback_data=HelpCallback(tab="games", user_id=user_id))
    ...
    b.button(text=label("⚙️ Управление", "settings"), callback_data=HelpCallback(tab="settings", user_id=user_id))
    if is_dev:
...
async def cmd_bot_alone(message: types.Message, developer_id: int = 0):
    if message.chat.type == "private":
        return await answer_group_only(message)
    await message.answer(
        HELP_PAGES["main"],
        reply_markup=get_help_keyboard("main", is_dev=bool(developer_id and message.from_user.id == developer_id), user_id=message.from_user.id),
        parse_mode="HTML",
    )
...
def _ai_knowledge_text() -> str:
    ...
    сейчас, самокорректируется и уже актуализировано под единый parity-контракт
    сайта и чата (в отличие от GAME_BIBLE.md, который не обновляется автоматически
    и местами устарел)."""
...
async def cmd_ai_question(message: types.Message, ai_question: str, db):
    ...
    if action_kb is not None:
        ...
        if section_hit:
            ...
            kb.button(
                text="📖 Полная справка",
                callback_data=HelpCallback(tab="main", user_id=message.from_user.id),
            )
            if _MINIAPP_URL:
