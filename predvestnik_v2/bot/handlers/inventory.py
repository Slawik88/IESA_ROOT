...
_ITEM_DETAILS: dict[str, dict] = {
    ...
    # Spin tokens
    "spin_token_novice": {
        "use":    "Используется автоматически при спине Ученической крутки — бесплатно",
        "get":    "Стрик-блок (каждые 7 дней), достижения",
        "tip":    "Кнопка крутки показывает 'БЕСПЛАТНО' если жетон есть",
        "cmds":   ["бот крутка"],
    },
    "spin_token_standard": {
        "use":    "Используется автоматически при спине Стандартной крутки",
        "get":    "Стрик-блоки, достижения, ивенты",
        "tip":    "Стандартная крутка — шансы лучше Ученической",
        "cmds":   ["бот крутка"],
    },
    "spin_token_premium": {
        "use":    "Используется автоматически при спине Премиум крутки",
        "get":    "Редкие достижения, мировые ивенты",
        "tip":    "Один из лучших способов получить редкий лут",
        "cmds":   ["бот крутка"],
    },
    "spin_token_diamond": {
        "use":    "Используется автоматически при спине Алмазной крутки",
        "get":    "Только из очень редких ивентов и достижений",
        "tip":    "Алмазная крутка даёт самые мощные предметы",
        "cmds":   ["бот крутка"],
    },
    # Eggs
    "egg_basic": {
    ...
    "slot_expander": {
        "use":    "Используй через «бот зоопарк» для расширения питомника",
        "get":    "Магазин за 10 💎",
        "tip":    "Постоянно +1 слот (максимум 6 расширений)",
        "cmds":   ["бот зоопарк", "бот магазин"],
    },
    "shield_day": {
