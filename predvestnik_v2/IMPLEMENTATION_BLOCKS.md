...
| 5 | ✅ **ВЫПОЛНЕНО** Браки 2.0 (кошелёк 4 валюты, подарок партнёру, каталог подарков, оплата семьёй) | 🟡 средний | Блок 4 |
| — | ✅ **Навигация 6→5** (UX-лимит 3-5): Пропуск → «Ещё» + быстрые-кнопки Профиля | 🟢 низкий | — |
| 6 | ✅ **ВЫПОЛНЕНО** БП-админка: гибкая настройка наград + выбор из 2 наград | 🟡 средний | — |
| 7 | ✅ **ВЫПОЛНЕНО** Акции дня: ротация 12ч, 3-7 слотов, скидка 5-50% | 🟡 средний | — |
| 8 | ✅ **ВЫПОЛНЕНО** Гача: 4 тира → 2 режима (mora/diamond) + единый жетон | 🔴 высокий | — |
| 9 | 🟦 **ЧАСТИЧНО** Магазин: +3 еды (data-driven) ✅; спец-механики отложены | 🟡 средний | Блок 8 |
| 10 | ✅ **ВЫПОЛНЕНО** Онбординг новичка (стартовый набор) | 🟢 низкий | Блок 8 |
| 12 | ✅ **ВЫПОЛНЕНО** Питомцы active/passive (owl/unicorn/hamster/turtle/dragon/dog/falcon/fox; wolf исключён) | 🟡 средний | — |
| 13 | 🟦 **ЧАСТИЧНО** Реликвии (сток Тёмной Моры) ✅; ивенты/теневые реликвии ⏳ | 🟡 средний | — |
| 14 | Годовщины брака | 🟢 низкий | — |
| 15 | ✅ **ВЫПОЛНЕНО** Настройки уведомлений (vip_expiry, bp_reminder) | 🟡 средний | — |
...
## Навигация — редизайн 6→5 вкладок (UX-лимит 3-5) ✅ ВЫПОЛНЕНО

**Задача:** нижнее меню должно быть строго 3-5 вкладок, глубина ≤3 клика,
segmented controls для под-логики, без гамбургеров.
**Было:** 6 вкладок (Профиль, Питомцы, Арена, Пропуск, Магазин, Ещё) — нарушение.
**Стало:** 5 вкладок — 👤 Профиль · 🐾 Питомцы · ⚔️ Арена · 💎 Магазин · 🎛 Ещё.
- **Пропуск** демоутнут (сезонная прогрессия): карточка в «Ещё» (1-й, выделенная)
  + быстрые-кнопки на Обзоре Профиля → видимость сохранена, глубина 2 клика.
- Внутри разделов — segmented controls (Профиль: Обзор/Инвентарь/Темы/Квесты/
  Ачивки/Топ; Магазин: Гача/Премиум/Расходники/Акции; Питомцы: Мои/Склад/Бестиарий).
- Шрифт нав-бара возвращён 8px→9px (5 вкладок помещаются комфортно).

---

## Блок 5 — Браки 2.0: family wallet, модалка оплаты, подарки ✅ ВЫПОЛНЕНО

**Зависимости:** Блок 4 (мультивалютные транзакции). Все 4 сабблока сделаны.
...
### ✅ Сабблок 4 — каталог подарков партнёру (ВЫПОЛНЕНО)
**Файлы:** `core/registry.py` (`PARTNER_GIFTS`), `bot/core/database.py`
(таблица `partner_gifts_log`), `infrastructure/repositories/marriages.py`
(`purchase_partner_gift`, `get_received_gifts`), `bot/handlers/marriage.py`,
`FastAPI/routers/marriage.py`, `FastAPI/static/app.js`.
- 6 подарков: 3 косметических (🌹/🧸/🍫) + 3 бафф-подарка (📚/💍/👑).
- Бафф-подарки накладывают `study_xp` на ПАРТНЁРА (реальный баф — потребляется
  в `services/leveling.py`, +XP на 6/12/24ч). UPSERT с GREATEST(срок, value).
- `бот подарки` — каталог с кнопками; покупка `GiftBuyCB` атомарна (FOR UPDATE
  на балансе дарителя + wallet_log аудит). Оплата с личного баланса.
- Сайт: карточка брака показывает «🎁 Подарки от партнёра» (последние 5).

### ✅ Сабблок 2 — оплата из семейного кошелька (ВЫПОЛНЕНО, авто-перевод)
**Решение:** авто-перевод (выбран как самый безопасный, без переписи логики
списания в каждой покупке).
- Эндпоинт `POST /marriage/fund` — переносит сумму family→личный перед покупкой
  (marriage_id резолвится сервером; излишек от скидки остаётся на личном — деньги
  не теряются).
- Фронт-хелпер `withPaymentSource(currency, amount, proceed)`: не женат → сразу
  личный (без модалки); женат → модалка «👤 Личный / 💑 Семейный», при семейном
  авто-перевод затем покупка.
- Подключено: покупка слота питомника (референс) + **VIP** (confirm → выбор
  источника → покупка), оба через `withPaymentSource`.
- ⏳ Осталось (опц.): магазин (цена не на call-site — нужно протянуть в кнопку),
  гача (жетоны бесплатных круток + мульти — своя логика), темы (lookup цены).
  Каждая — bespoke-флоу; раскатывать по мере надобности тем же враппером.

---

## Блок 6 — БП-админка: гибкая настройка наград ✅ ВЫПОЛНЕНО

**Файлы:** `bot/core/database.py` (колонка `reward_options` + миграция),
`services/battle_pass.py` (`claim_reward(choice_index)`, `get_level_options`,
`_opt_to_reward`, `reward_short_text`), `FastAPI/routers/dev_console.py`
(set/get + bulk/copy/summary), `FastAPI/routers/battle_pass.py`,
`bot/handlers/battle_pass.py`, `FastAPI/static/app.js`.

**Что сделано**
1. ✅ Выбор ЛЮБОЙ награды на уровень/трек — было (эндпоинты `/bp/rewards`).
   Добавлен **dev-UI редактора наград** (его раньше не было — только бэкенд):
   season/level/track + мора/алмазы/предметы/тема, кнопки Сохранить/Сброс.
2. 🆕 **Две награды = выбор игрока**: колонка `reward_options` (JSON ≥2 вариантов).
   `claim_reward(..., choice_index)` грантит выбранный вариант; `get_level_options`
   отдаёт варианты. Бот: клик «Забрать» на choice-уровне → кнопки вариантов
   (`BpChoiceCB`). Сайт: статус отдаёт `options`, кнопка «Выбрать» → модалка-пикер.
   В dev-UI — чекбокс «🔀 сделать выбором из 2 наград».
3. 🆕 **Доп. функции админки**:
   - `/bp/rewards/bulk` — массовое автозаполнение диапазона по формуле base+step×lv.
   - `/bp/rewards/copy` — копирование всех наград из сезона в сезон (шаблон).
   - `/bp/season-summary` — сводная «ценность сезона» (сумма мора/алмазы/предметы
     по трекам) + список пустых уровней (подсветка пропущенных). Кнопка в dev-UI.

**Проверка:** `py_compile` 5 файлов + `node --check` + `BpChoiceCB` round-trip (18B).
...
## Блок 8 — Гача: 4 тира → 2 режима ✅ ВЫПОЛНЕНО

**Как сделано (движок уже был generic по `spin_type` — меняли только данные):**
- `spin_type`: `novice/standard/premium/diamond` → **`mora`/`diamond`** во ВСЕХ
  dict (`SPIN_COSTS`, `PITY_SOFT/HARD`, `SPIN_TYPE_LABELS`, `SPIN_TOKEN_IDS`,
  `GACHA_TABLES`, `PITY_HARD_REWARD`, `THEME_GACHA_DROPS`, `THEME_DROP_CHANCES`).
- `GACHA_TABLES`: 2 новые таблицы. **mora** (~600🪙) — common→epic, мора,
  расходники, малый шанс 💎 (22% ценных). **diamond** (~8💎) — выше шанс +
  шире диапазон (100% ценных: epic 12%, legendary 4%, золотые/мифические/
  кристальные яйца, гарантии 💎). Пити: mora 30/60→epic, diamond 18/35→legendary.
- **Единый жетон**: 4 старых `spin_token_*` → 1 `spin_token` (бесплатный спин
  мора-режима). Все ~35 точек выдачи (VIP/БП/ачивки/daily-deal/вехи/стрик/квесты)
  перепривязаны (гранты аддитивны — дубли в бандле суммируются). Алмазный режим —
  только за 💎 (без жетона).
- **Миграция БД** (идемпотентная): суммирует старые 4 жетона в инвентарях → один
  `spin_token`, удаляет старые. Старые pity-строки безвредны (новые ключи).
- UI: бот `_SPIN_ORDER=[mora,diamond]` + алиасы команд; FastAPI-роутер и
  app.js уже генерик по списку режимов (правки иконок/имён/лейблов).
- **Проверка:** py_compile (8) + node --check + cross-consistency тест (ключи
  выровнены; ВСЕ granted/gacha item_id есть в ITEMS_REGISTRY; legacy-жетоны
  исчезли) + распределение лута.

---

### (исходный план) ⚠️ САМЫЙ РИСКОВАННЫЙ БЛОК — делать когда всё остальное стабильно
...
## Блок 10 — Онбординг новичка ✅ ВЫПОЛНЕНО

**Сделано:**
- `services/onboarding.grant_starter_kit` — выдаёт РОВНО раз: 🐾 1 common-питомец
  (`grant_duplicate`), 🪙 1000 Моры, 💎 8 Алмазов (= 1 алмазный спин), 🎟 1
  `spin_token` (1 мора-спин). Атомарный guard `UPDATE … WHERE onboarded=FALSE
  RETURNING` (идемпотентно даже при гонке сообщений).
- Колонка `users.onboarded` DEFAULT TRUE (миграция) → существующие игроки набор
  НЕ получают; `users.update_user` вставляет новых с `onboarded=FALSE`.
- Хук в `bot/middlewares/db.py` после `update_user`: выдаёт набор + приветствие в
  чат (fire-and-forget); для существующих — no-op.
- Алмазный спин выдан как 8💎 (в Блоке 8 у алмазного режима нет жетона —
  оплачивается алмазами).

**Проверка:** py_compile + сверка с механикой add_balance/add_item/grant_duplicate.

---

### (исходный план) Онбординг новичка
...
## Блок 12 — Питомцы: гибридная роль активный/пассивный 🟦 ЧАСТИЧНО

**Сделано (фундамент + 2 чистых сайта):**
- `core/constants.scale_pet_bonus(bonus, placement)` — passive → числовые ×0.5,
  булевы capstone → off; `trigger_every_n_msg` (инверсное) НЕ масштабируется
  (иначе делитель→0). Проверено на owl-landmine.
- `zoo.get_species_level_placement` (приоритет active) + `zoo.get_species_bonus`
  (готовый масштабированный бонус).
...
## Блок 12 (исходный план) — Питомцы: гибридная роль
...
## Блок 15 — Настройки уведомлений ✅ ВЫПОЛНЕНО

**Файлы:** `bot/core/database.py` (таблица `user_notification_prefs`),
`core/constants.py` (`NOTIFICATION_CATEGORIES`),
`infrastructure/repositories/notifications.py` (is_enabled/get_prefs/set_pref/toggle),
`bot/handlers/notifications.py` (команда + тоггл), `services/scheduler.py` (гейтинг).

**Что сделано**
- Таблица `user_notification_prefs(user_id, category, enabled)` — нет строки =
  включено по умолчанию.
- Команда `бот уведомления` (`настройки уведомлений`) — инлайн-тогглы по
  категориям, callback `NotifCB` переключает и перерисовывает.
- Гейтинг персональных DM в scheduler: **истечение VIP** (`vip_expiry`) и
  **конец сезона БП** (`bp_reminder`) — шлются только если включено; флаги
  `expiry_notified`/`season_end_notified` ставятся всё равно (без спама).
- Групповые сообщения в чат (сундуки, годовщины) — НЕ пер-юзерные (одно
  сообщение на чат), не управляются; в UI это явно указано. Категории легко
  расширяются через `NOTIFICATION_CATEGORIES`.

**Проверка:** py_compile 6 файлов.

---

## Блок 15 (исходный план) — Настройки уведомлений
