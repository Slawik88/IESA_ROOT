...
> Текущее состояние всех систем — `GAME_BIBLE.md`.
>
...
> Поверх блоков прошёл аудит (файл аудита удалён 2026-07-07; итоги — в GAME_BIBLE.md) — критичные баги
...
2. **Кланы/гильдии — ✅ СДЕЛАНО (2026-06-23).** Полная вертикаль: `core/constants`
   (CLAN_*), таблицы `clans`/`clan_members` (`database.py._init_clans` + `clans.ensure_tables`),
   репо `infrastructure/repositories/clans.py`, сервис `services/clans.py` (единый
   источник правды), веб-роутер `FastAPI/routers/clans.py` (`/clans/` · create · join ·
   leave) + UI «🛡 Клан» на профиле (создание/состав/топ/выход), бот `bot/handlers/clans.py`
   (`бот клан` · `кланы` · `клан создать, Имя ТЕГ` · `клан выйти` + inline-вступление).
   Соц-прогрессия, НЕ P2W. Проверено: DDL на живом PG17 (rollback), import FastAPI.main +
   bot.handlers, openapi-роуты, render-скриншот UI.
...
*Конец плана. Текущее состояние всех систем — `GAME_BIBLE.md`.
