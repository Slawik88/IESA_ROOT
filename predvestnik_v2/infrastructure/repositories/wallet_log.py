...
async def log_wallet(
    ...
    *,
    ...
    if delta_mora == 0.0 and delta_diamonds == 0.0:
        return
    ...
    await db.execute(
        """INSERT INTO wallet_log
            (user_id, chat_id, delta_mora, delta_diamonds,
             balance_mora_after, balance_diamonds_after,
             source, target_id, note)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (user_id, chat_id, delta_mora, delta_diamonds,
         bal_mora, bal_dia, source, target_id, note),
    )
...
async def get_recent(
    ...
    if sources:
        ...
        query = (
            f"SELECT id, delta_mora, delta_diamonds, balance_mora_after, "
            f"balance_diamonds_after, source, target_id, note, created_at "
            f"FROM wallet_log WHERE user_id = ? AND source IN ({placeholders}) "
            f"ORDER BY created_at DESC LIMIT ?"
        )
        params = (user_id, *sources, n)
    else:
        query = (
            "SELECT id, delta_mora, delta_diamonds, balance_mora_after, "
            "balance_diamonds_after, source, target_id, note, created_at "
            "FROM wallet_log WHERE user_id = ? "
