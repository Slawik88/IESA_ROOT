...
# Smart Checkout может покрыть только дефицит Моры. Дефицит Алмазов не
# конвертируется: эту валюту игрок получает за испытания и сезонные рубежи.
ZARNIKI_EXCHANGE_RATES: dict = {
    "mora": ZARNIKI_TO_MORA_RATE,
}
...
# 'artifact' добавлен 2026-07-29 (аудит вкладки «Внешний вид» после перехода на линейки) —
# без него .get(rarity, ...) падал на дефолт для топ-линейки «⚡ Артефакт». Из сундуков
# artifact-редкость сейчас не дропается (см. COSMETIC_CHESTS — только common/rare/epic),
# так что это защитный задел на будущее, не боевой путь.
COSMETIC_DUPE_SHARDS: dict = {"common": 3, "rare": 6, "epic": 12, "legendary": 25, "mythic": 50, "artifact": 75}
# Крафт косметики из осколков — стоимость по редкости (только source=="shop"):
# 'artifact' добавлен 2026-07-29 — БЕЗ него craft_catalog() отдавал fallback 9999🔹 для
# 13 предметов линейки «⚡ Артефакт» (все source=="shop", то есть реально в списке крафта) —
# крафт для них выглядел сломанным/недостижимым. Прод-баг, не гипотетический.
COSMETIC_CRAFT_SHARDS: dict = {"common": 10, "rare": 25, "epic": 60, "legendary": 150, "mythic": 400, "artifact": 650}
...
GRID_W: int = 9                 # ширина поля (колонки x=0..8)
GRID_H: int = 7                 # высота поля (ряды y=0..6)
...
B4_ENEMY_MOVE: int = 3          # на сколько клеток враг может сместиться за ход (EV-ИИ)
...
# Онбординг боя (2026-07-21): пошаговая анимация хода врага — тайминги «ленты битов».
# Сервер отдаёт упорядоченный timeline, клиент проигрывает бит за битом (см. app.11.js).
# Значения — зеркалятся JS-константами B4_BEAT_* в app.11.js; правишь тут — правь и там.
B4_BEAT_MOVE_MS: int = 300           # длительность слайда врага на клетку
B4_BEAT_HIT_MS: int = 220            # вспышка/число урона на удар
B4_BEAT_GAP_MS: int = 180            # пауза между битами (читаемость серии)
# Скриптованный «Первый бой»: фикс-сид → детерминированное поле и поведение врага,
# чтобы коуч-подсказки совпадали с тем, что реально произойдёт.
B4_TUTORIAL_SEED: int = 20260721
