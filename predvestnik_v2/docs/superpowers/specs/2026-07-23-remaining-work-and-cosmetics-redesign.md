...
**Статус:** пункт 1 (баг) пофикшен по прямому указанию владельца, ждёт деплоя.
Всё остальное ЖДЁТ ОТДЕЛЬНОГО «ДЕЛАЕМ» — кода по этим пунктам нет и не будет.
...
## 3. 🎨 Косметика: проработка эффектов по редкости
Каждый предмет косметики должен быть проработан индивидуально — **чем выше редкость и
цена, тем сложнее и заметнее визуальный эффект** (не одна и та же по силе анимация на
всех рарностях с разницей только в цвете рамки). Ожидаемый результат: common — простой
статичный акцент, вплоть до mythic/shadow — многослойный эффект (частицы, движение,
свечение), чтобы цена ощущалась глазами. Прохожусь по каждому слоту (ореол/рамка/гало/
фон/частицы/титул) и по каждому предмету внутри.
