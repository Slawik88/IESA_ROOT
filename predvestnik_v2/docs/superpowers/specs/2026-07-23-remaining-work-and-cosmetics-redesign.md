...
вёрстки тем) сделаны; пункт 8 (mythic-темы) оказался уже реализован раньше — правки
не понадобилось. Всё это по прямому указанию владельца, ждёт деплоя. Пункт 5 (цены)
явно пропущен по команде владельца — НЕ трогать. Всё остальное (6, 9, 11–14, 16)
ЖДЁТ ОТДЕЛЬНОГО «ДЕЛАЕМ». Пункт 19 (опечатки/кодировка) — явно последний по
приоритету среди ВСЕХ блоков.
