...
## БЛОК 30: РЕФАНД КОСМЕТИКИ И НОВАЯ ЦЕНОВАЯ ИЕРАРХИЯ

> Источник: запрос 2026-06-30.

### 30.1 — Одноразовый скрипт рефанда (КРИТИЧНО — реальные деньги игроков)
Игроки покупали косметику по СТАРЫМ ценам (с двумя валютами — мора+зарники или Stars+зарники).
Нужен одноразовый скрипт, который:
1. Находит все покупки косметики из `user_cosmetics` с историей из `wallet_history` / `cosmetic_purchases` (или аналогичной таблицы логов)
2. Определяет, что именно и сколько было потрачено (зарники / Stars / мора) по СТАРЫМ ценам
3. Возвращает ресурсы игроку точно в той валюте, в которой было потрачено
4. Снимает купленный предмет косметики (UPDATE user_cosmetics SET is_owned=FALSE / DELETE)
5. Логирует каждый рефанд в `admin_grant_log`
6. Запускается ОДИН РАЗ вручную командой (не в scheduler, не при старте)

**Старые цены (до правок):**
| Предмет | Старая цена |
|---------|-------------|
| Серебряный ореол | 250 ✨ зарников |
| Золотое сияние | 450 ✨ зарников (VIP) |
| Багровое пламя | 750 ✨ зарников (VIP) |
| Призматический ореол | 1250 ✨ + VIP |
| Разлом Бездны | 2000 ✨ + VIP |

> ⚠️ Сначала: точно выяснить схему хранения покупок косметики (какая таблица, какие поля) — сделать SELECT и сверить с реальностью. Только потом писать скрипт.

### 30.2 — Новая ценовая иерархия (применить ко всем предметам косметики)
```
Обычный    → 250 ✨
Редкий     → 440 ✨
Эпический  → 630 ✨
...
Мифический  → 1000 ✨
```
- Применить в `core/registry.py` (COSMETICS_REGISTRY или аналог) ко ВСЕМ предметам
- Обновить UI-карточки на сайте (цена отображается динамически с бэкенда — проверить)
- Убрать ценовые поля для Stars полностью (пункт 30.3)

### 30.3 — Убрать покупку за Stars полностью ✅ → к реализации
- В UI на сайте убрать кнопку «Купить за X Stars» из карточки косметики
- В бэкенде: проверить, есть ли endpoint для Stars-оплаты косметики — если да, убрать или отключить
- `bot/handlers/payments.py` или аналог — проверить, есть ли route для Stars-косметики

### 30.4 — Косметика в БП: двойной доступ
- Косметика в наградах БП остаётся
- НО её можно ТАКЖЕ купить за зарники по обычной иерархии (30.2)
- Убрать косметику из наград за ачивки (или оставить как «альтернативный» источник — уточнить у пользователя)
