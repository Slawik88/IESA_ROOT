...
client = ((ROOT / "FastAPI/static/app.04.js").read_text(encoding="utf-8")
          + (ROOT / "FastAPI/static/app.06.js").read_text(encoding="utf-8"))
...
# Archive-only duel recovery must not leak a public interface after the approved
# removal of the former combat system.
assert "Вызвать игрока на дуэль" not in client
assert "Освободить ставку" not in client
assert "Новые бои проходят во вкладке «Игра»" not in client
