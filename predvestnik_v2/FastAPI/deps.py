...
from FastAPI.auth import (verify_preprod_browser_ticket, verify_webapp_data,
                          verify_session_token, hash_signal)
...
async def require_tg_user_base(
    ...
    x_preprod_browser_ticket: str = Header(default=""),
    ...
    # 2. Local automation persona. This cookie can only be issued by a
    # second Uvicorn listener bound to 127.0.0.1, and is rejected everywhere
    # except isolated preprod. It intentionally wins over a stale browser
    # session header, but never over genuine Telegram initData above.
    # Do not turn this into a production fallback.
    local_test_persona = False
    if not user and is_preprod():
        test_user_id = verify_session_token(request.cookies.get(_PREPROD_TEST_COOKIE, ""))
        if test_user_id and is_preprod_browser_test_user(test_user_id):
            user = {"id": test_user_id, "preprod_test_persona": True}
            local_test_persona = True

    # The visible browser automation surface can partition a localhost cookie
    # when it follows the 8404 -> 8403 redirect.  It receives only a short,
    # purpose-bound test ticket generated by the loopback listener; regular
    # clients never send this header and no real Telegram identity is accepted.
    if not user and is_preprod() and x_preprod_browser_ticket:
        test_user_id = verify_preprod_browser_ticket(x_preprod_browser_ticket)
        if test_user_id and is_preprod_browser_test_user(test_user_id):
            user = {"id": test_user_id, "preprod_test_persona": True}
            local_test_persona = True

    # 3. Login Widget session (browser)
    ...
    if not local_test_persona and not require_preprod_user(int(user["id"])):
...
async def require_tg_user(user=Depends(require_tg_user_base), db=Depends(get_db)):
    ...
    if account and account[0] is not None:
        raise HTTPException(
            status_code=403,
