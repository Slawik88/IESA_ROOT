...
(function(){
  ...
  window.openPetsV1=function(){
    ...
    api('/pets-v1/me').then(d=>{
      ...
      root.innerHTML=`<div class="looks-head"><button class="looks-back" onclick="goTo('profile')" aria-label="Назад">‹</button><div class="looks-htitle">🐾 Питомцы</div></div><div class="looks-hint">Выносливость уменьшается примерно на 10 в сутки. Еда из сундуков восстанавливает её сразу; смена активного питомца стоит 5.</div>${earned}${cards||'<div class="empty">Первую карточку питомца можно найти в сундуке.</div>'}${timer}`;
