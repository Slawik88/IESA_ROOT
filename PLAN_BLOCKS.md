...
## БЛОК 8 — Глобальный баф чата (+10% XP, 1 час)

**Файлы**:
- `PredvestnikBot/database/db.py` — `activate_chat_buff()`, `get_active_chat_buff()`, `cleanup_expired_chat_buffs()`
- `PredvestnikBot/api/economy.py` — `buy_chat_buff(uid, chat_id, buff_type)`
- `PredvestnikBot/middlewares/message_counter.py` — проверка бафа при начислении XP
- `PredvestnikBot/web/index.html` — таймер бафа + кнопка покупки
- `IESA_ROOT/IESA_ROOT/miniapp_views.py` — `/api/chat_buff` GET + POST

### 8.1 — `activate_chat_buff()` в db.py
```python
async def activate_chat_buff(chat_id, buff_type, activated_by, duration_minutes=60):
    # UPSERT в chat_global_buffs (UNIQUE chat_id + buff_type)
    # Если баф уже активен — добавить в очередь? Нет — разрешить только если закончился.
    expires = datetime.now(utc) + timedelta(minutes=duration_minutes)
    INSERT INTO chat_global_buffs (...) ON CONFLICT DO UPDATE SET ...
    -- только если текущий expires_at < NOW()
```

### 8.2 — Проверка бафа в middleware
В `message_counter.py`, при начислении XP:
```python
chat_buff = await get_active_chat_buff(chat_id, "xp_plus10")
if chat_buff:
    xp_amount = int(xp_amount * 1.1)  # +10%
```

### 8.3 — `buy_chat_buff(uid, chat_id, "xp_plus10")`
```python
CHAT_BUFF_PRICE = 1500  # 🪙
# 1. Списать мору
# 2. activate_chat_buff(chat_id, "xp_plus10", uid, 60)
# 3. Анонс в чат: "⚡ [имя] активировал баф! +10% XP для всех на 1 час!"
# 4. Вернуть {ok, expires_at}
```

### 8.4 — Показ таймера в мини-апп
На главном экране или в разделе "Бонусы":
```
⚡ Активный баф чата: +10% XP
⏳ Осталось: 43 мин
```
Если нет активного бафа:
```
[кнопка: Активировать +10% XP на 1 час — 1500 🪙]
```

---

## СВОДНАЯ ТАБЛИЦА ФАЙЛОВ ПО БЛОКАМ

| Блок | Основные файлы |
|------|---------------|
| 1 | `database/db.py` (миграции) |
| 2 | `middlewares/message_counter.py`, `database/db.py`, `handlers/user.py`, `web/index.html` |
| 3 | `handlers/dev_panel.py`, `database/db.py`, `api/admin.py`, `utils/scheduler.py`, `web/index.html`, `miniapp_views.py` |
| 4 | `shared_prices.py`, `database/db.py`, `api/gacha.py`, `web/index.html`, `miniapp_views.py` |
| 5 | `shared_prices.py`, `api/gacha.py`, `api/expeditions.py`, `web/index.html`, `miniapp_views.py` |
| 6 | `shared_prices.py`, `api/shop.py`, `web/index.html`, `miniapp_views.py` |
| 7 | `api/economy.py`, `database/db.py`, `handlers/wallet.py`, `web/index.html`, `miniapp_views.py`, `utils/scheduler.py` |
| 8 | `database/db.py`, `api/economy.py`, `middlewares/message_counter.py`, `web/index.html`, `miniapp_views.py` |
