...
## BLOCK 1 — Онбординг: первый день пользователя ⚡💅
...
- [ ] Определять первый вход: добавить поле `User.onboarded = BooleanField(default=False)`
- [ ] При `onboarded=False` → показывать **welcome-modal** (поверх профиля):
  - Большая иконка 🏔️ + «Добро пожаловать в IESA Sport!»
  - Краткое объяснение (3 буллета): что такое карта, PIN, партнёры
  - CTA-кнопка «Начать → Заполнить профиль» + ссылка «Пропустить»
  - После закрытия: `onboarded=True` через HTMX POST (не перезагружая страницу)
- [ ] CSS: анимированный backdrop + pop-in эффект для модала

### 1b — Progress-бар заполнения профиля ⚡💅
- [ ] На странице профиля (под хедером) — компактная горизонтальная полоска прогресса:
  - Метрика: `completeness = (avatar + phone + bio + telegram + dob) / 5 × 100%`
  - Цвет: red → amber → green при 100%
  - Текст рядом: «Профиль заполнен на 60% — осталось 2 шага»
  - Клик на «шаги» → разворачивает список незаполненных полей со ссылками
- [ ] HTMX-обновление: пересчёт % без перезагрузки при изменении данных
- [ ] Скрывать полосу при 100% (не надоедать)

### 1c — Contextual tooltips для новых юзеров 💅
- [ ] На странице профиля: первые 7 дней показывать subtle-подсказки (aria-описания):
  - Рядом с PIN-картой: «Это ваш личный код — показывайте его на стойке партнёра»
  - Рядом с QR: «Отсканируйте в любом партнёрском заведении»
  - Рядом с Telegram: «Привяжите TG — получайте мгновенные уведомления о визитах»
- [ ] Реализация: CSS `[data-tip]` pseudo-элемент, закрываемые через `localStorage`
- [ ] Не показывать пользователям с `membership_status='active'` и `total_visits > 3`

### 1d — «Быстрые действия» после онбординга 💅
- [ ] На профиле — горизонтальный scroll-carousel «Что можно сделать» (только до 3-го визита):
  - 🔗 Привязать Telegram → link
  - 📷 Добавить фото профиля → link
  - 🗺️ Найти ближайшего партнёра → link
  - 📝 Написать первый пост → link
- [ ] После выполнения каждого пункта — анимированная галочка (HTMX check)

### 1e — Страница «Как это работает» для новых членов
- [ ] Отдельная страница `/about/how-it-works/` (или секция на главной под хедером):
  - 4 шага с анимированными иконками: Регистрация → Активация → Визит → Привилегии
  - Интерактивная: при скролле шаги «загораются» один за другим
- [ ] Ссылка в navbar для неаутентифицированных пользователей + в welcome-modal
