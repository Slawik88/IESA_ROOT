/* в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
   Season.tsx вЂ” Р‘РѕРµРІРѕР№ РџСЂРѕРїСѓСЃРє (Battle Pass) вЂ” Р РµРґРёР·Р°Р№РЅ
   GET /api/season/data
...
export default function Season() {
  ...
  const handleClaim = useCallback(async (level: number, isPremium: boolean) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast(`рџЋЃ РќР°РіСЂР°РґР° РЈСЂ.${level}${isPremium ? " (Premium)" : ""} РїРѕР»СѓС‡РµРЅР°!`, "success");
        setData(prev => {
          if (!prev) return prev;
        ...
        toast(res.error ?? "РќРµРґРѕСЃС‚СѓРїРЅРѕ", "warning");
      }
    } catch (e: unknown) {
      toast(e instanceof Error ? e.message : "РћС€РёР±РєР°", "error");
    } finally {
      setBusy(null);
    }
  }, [data, busy, toast]);
  ...
  const handleBuyPremium = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast("рџ‘‘ Premium РїСЂРѕРїСѓСЃРє Р°РєС‚РёРІРёСЂРѕРІР°РЅ!", "success");
        setData(prev => prev ? { ...prev, progress: { ...prev.progress, has_premium: true } } : prev);
      } else {
        toast(res.error ?? "РќРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ СЃСЂРµРґСЃС‚РІ", "error");
      }
    } catch (e: unknown) {
      toast(e instanceof Error ? e.message : "РћС€РёР±РєР°", "error");
    } finally {
  ...
  /* в”Ђв”Ђ РЎРѕСЃС‚РѕСЏРЅРёСЏ в”Ђв”Ђ */
  if (error === "No active season" || (error && error.includes("404"))) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-4 p-6 text-center"
        ...
        <p className="font-semibold text-lg" style={{ color: "var(--text-primary)" }}>РЎРµР·РѕРЅ РЅРµ Р°РєС‚РёРІРµРЅ</p>
        <p className="text-sm max-w-[240px]">РЎР»РµРґРёС‚Рµ Р·Р° Р°РЅРѕРЅСЃР°РјРё вЂ” РЅРѕРІС‹Р№ СЃРµР·РѕРЅ СЃРєРѕСЂРѕ РЅР°С‡РЅС‘С‚СЃСЏ!</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fadeIn pb-4" style={{ backgroundColor: "var(--bg-primary)" }}>
      ...
      {/* в”Ђв”Ђ HERO вЂ” РЁР°РїРєР° СЃРµР·РѕРЅР° в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      <div className="relative overflow-hidden glass-hero px-4 pt-safe pb-4">
        {/* Р¤РѕРЅРѕРІС‹Р№ РґРµРєРѕСЂ */}
          ...
          <div>
            ...
            {season.end_date && (
              <p className="text-[11px] mt-0.5" style={{ color: "var(--text-hint)" }}>
                Р”Рѕ: {new Date(season.end_date).toLocaleDateString("ru-RU", { day: "numeric", month: "long" })}
              </p>
            )}
          </div>
          {!hasPremium ? (
        ...
        <div className="relative">
          <div className="flex items-end justify-between mb-2">
            <div className="flex items-baseline gap-1.5">
              ...
              <span className="text-sm font-semibold" style={{ color: "var(--text-hint)" }}>СѓСЂРѕРІРµРЅСЊ</span>
            </div>
            <div className="text-right">
              ...
              <p className="text-[10px]" style={{ color: "var(--text-hint)" }}>
                {claimedTotal} / {maxRewards} РЅР°РіСЂР°Рґ
              </p>
            </div>
          </div>
      ...
      {/* в”Ђв”Ђ Р›РµРіРµРЅРґР° в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      <div className="flex items-center gap-4 px-4 py-2.5">
        <div className="flex items-center gap-1.5 text-xs" style={{ color: "var(--text-hint)" }}>
          <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: "#22c55e" }} />
          <span>Р‘РµСЃРїР»Р°С‚РЅС‹Р№</span>
        </div>
        <div className="flex items-center gap-1.5 text-xs" style={{ color: "var(--text-hint)" }}>
      ...
      {/* в”Ђв”Ђ РўСЂРµРє РЅР°РіСЂР°Рґ в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
...
/* в”Ђв”Ђ РљР°СЂС‚РѕС‡РєР° РѕРґРЅРѕРіРѕ СѓСЂРѕРІРЅСЏ в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
...
function BPRewardCard({
  ...
  const freeLabel = reward.free_reward    ?? (freeMora > 0 ? `+${freeMora} рџЄ™` : freeXp > 0 ? `+${freeXp} XP` : null);
  const premLabel = reward.premium_reward ?? (premMora > 0 ? `+${premMora} рџЄ™` : premXp > 0 ? `+${premXp} XP` : null);
...
function EmptyCell() {
  return (
    <div className="rounded-xl p-2.5 flex items-center justify-center"
      ...
      <span className="text-[10px]" style={{ color: "var(--text-hint)" }}>вЂ”</span>
    </div>
  );
}

/* в”Ђв”Ђ РЇС‡РµР№РєР° С‚СЂРµРєР° в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
    ...
    >
      ...
      {claimed ? (
        <div className="flex items-center gap-1 mt-0.5">
          ...
          <span className="text-[10px] font-semibold" style={{ color }}>РџРѕР»СѓС‡РµРЅРѕ</span>
        </div>
      ) : canClaim ? (
        <button
          onClick={onClaim}
        ...
        >
          {busy ? <Loader2 size={10} className="animate-spin inline" /> : "Р’Р·СЏС‚СЊ"}
        </button>
      ) : (
        <span className="text-[10px]" style={{ color: "var(--text-hint)" }}>
          {!available && unlocked ? "рџ‘‘ Premium" : "рџ”’"}
        </span>
      )}
    </div>
  );
...
/* в”Ђв”Ђ РЎРєРµР»РµС‚РѕРЅ в”Ђв”Ђ */
